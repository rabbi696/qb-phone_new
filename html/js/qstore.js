...
$(document).on('click', '.storeapp-remove', function(e){
    ...
    DRI.Phone.Data.Applications[AppData.app] = null;

    $.post('http://dri-phone_new/RemoveApplication', JSON.stringify({
        app: AppData.app
    }));
    setTimeout(function(){
        $.post('http://dri-phone_new/SetupStoreApps', JSON.stringify({}), function(data){
            SetupAppstore(data); 
        });
    }, 100);
});
...
$(document).on('click', '.download-password-accept', function(e){
    ...
    if (FilledInPassword == TemplatePassword) {
        ...
        $(".download-progressbar-fill").animate({
            ...
            $.post('http://dri-phone_new/InstallApplication', JSON.stringify({
                app: CurrentApp,
            }), function(Installed){
                if (Installed) {
                    var applicationSlot = $(".phone-applications").find('[data-appslot="'+Installed.data.slot+'"]');
                    var blockedapp = IsAppJobBlocked(Installed.data.blockedjobs, DRI.Phone.Data.PlayerJob.name)
                    if ((!Installed.data.job || Installed.data.job === DRI.Phone.Data.PlayerJob.name) && !blockedapp) {
                    ...
                    DRI.Phone.Data.Applications[Installed.data.app] = Installed.data;
