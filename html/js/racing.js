                ...
                if (InDistance) {
                    $.post('http://qb-phone_new/IsBusyCheck', JSON.stringify({
                        ...
                        if (!IsBusy) {
                            ...
                            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You are in an editor..", "#1DA1F2");
                        }
                    });
                }
            })
        ...
        if (Races !== null) {
            ...
            $.each(Races, function(i, race){
                if (race.LastLeaderboard.length > 0) {
                    var elem = '<div class="racing-leaderboard-item" id="leaderboard-item-'+i+'"> <span class="racing-leaderboard-item-name"><i class="fas fa-flag-checkered"></i> '+race.RaceName+'</span> <span class="racing-leaderboard-item-info">Klik voor meer details</span> </div>'
