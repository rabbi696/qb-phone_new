...
AddEventHandler('QBCore:Client:OnJobUpdate', function(JobInfo)
    if JobInfo.name == "police" and JobInfo.name == "police2" and JobInfo.name == "police3" and JobInfo.name == "police4" and JobInfo.name == "police5" and JobInfo.name == "police6" and JobInfo.name == "police7" and JobInfo.name == "police8" then
        SendNUIMessage({
            action = "UpdateApplications",
            JobData = JobInfo,
            applications = Config.PhoneApplications
        })
    elseif PlayerJob.name == "police" and PlayerJob.name == "unemployed" then
        SendNUIMessage({
            action = "UpdateApplications",
            JobData = JobInfo,
            applications = Config.PhoneApplications
...
AddEventHandler('qb-phone_new:client:TransferMoney', function(amount, newmoney)
    ...
    if PhoneData.isOpen then
        SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "QBank", text = "&euro;"..amount.." credited!", icon = "fas fa-university", color = "#8c7ae6", }, })
        SendNUIMessage({ action = "UpdateBank", NewBalance = PhoneData.PlayerData.money.bank })
    else
        SendNUIMessage({ action = "Notification", NotifyData = { title = "QBank", content = "&euro;"..amount.." credited!", icon = "fas fa-university", timeout = 2500, color = nil, }, })
    end
end)
...
AddEventHandler('qb-phone_new:client:GetMentioned', function(TweetMessage, AppAlerts)
    ...
    if not PhoneData.isOpen then
        SendNUIMessage({ action = "Notification", NotifyData = { title = "You are mentioned in a Tweet above!", content = TweetMessage.message, icon = "fab fa-twitter", timeout = 3500, color = nil, }, })
    else
...
RegisterNUICallback('FetchVehicleScan', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('qb-phone_new:server:ScanPlate', function(result)
        QBCore.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
            ...
            local vehicleInfo = QBCore.Shared.Vehicles[QBCore.Shared.VehicleModels[model]["model"]] ~= nil and QBCore.Shared.Vehicles[QBCore.Shared.VehicleModels[model]["model"]] or {["brand"] = "Brand..", ["name"] = ""}
...
RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    QBCore.Functions.TriggerCallback('qb-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            ...
            QBCore.Functions.Notify('You are too far from the race. Your navigation is set to the race.', 'error', 5000)
